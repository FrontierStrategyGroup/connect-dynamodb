...
module.exports = function (connect) {
    ...
    DynamoDBStore.prototype.get = function (sid, fn) {
        ...
        fibonacciBackoff.on('ready', function(number, delay) {
            self.client.getItem(params, function(err, result) {
                if (err) {
                    if (err.code === 'ProvisionedThroughputExceededException' || err.statusCode === 500) {
    ...
    DynamoDBStore.prototype.set = function (sid, sess, fn) {
        ...
        fibonacciBackoff.failAfter(10);
        fibonacciBackoff.on('ready', function(number, delay) {
            self.client.putItem(params, function(err, result) {
                if (err) {
                    if (err.code === 'ProvisionedThroughputExceededException' || err.statusCode === 500) {
                        fibonacciBackoff.backoff();
                    } else {
    ...
    DynamoDBStore.prototype.reap = function (fn) {
        var self = this;
        var now = Math.floor(Date.now() / 1000) + 999999999;
        var options = {
            endkey: '[' + now + ',{}]'
        };
        var params = {
        ...
        var wrappedReap = function() {
            ...
            fibonacciBackoff.on('ready', function(number, delay) {
                ...
                self.client.scan(params, function onScan(err, data) {
                    if (err) {
                        if (err.code === 'ProvisionedThroughputExceededException' || err.statusCode === 500) {
                            console.log('hit scan throughput cap');
                            fibonacciBackoff.backoff();
                        } else {
                            fibonacciBackoff.reset();
                            return fn && fn(err);
    ...
    DynamoDBStore.prototype.destroy = function (sid, fn) {
        ...
        fibonacciBackoff.failAfter(10);
        fibonacciBackoff.on('ready', function(number, delay) {
            self.client.deleteItem(params, function(err, result) {
                 if (err) {
                    if (err.code === 'ProvisionedThroughputExceededException' || err.statusCode === 500) {
                        console.log('delete throughput cap hit');
                        fibonacciBackoff.backoff();
                    } else {
                        fibonacciBackoff.reset();
                        return fn && fn();;
